...
{
	s_vector	*v = new_vector(0, 0, 0);

	v->v_x = vec->v_x * m.matrix[0][0] + vec->v_y * m.matrix[1][0] + vec->v_z * m.matrix[2][0] + m.matrix[3][0];
	v->v_y = vec->v_x * m.matrix[0][1] + vec->v_y * m.matrix[1][1] + vec->v_z * m.matrix[2][1] + m.matrix[3][1];
	v->v_z = vec->v_x * m.matrix[0][2] + vec->v_y * m.matrix[1][2] + vec->v_z * m.matrix[2][2] + m.matrix[3][2];
	return (v);
}

// сначала мы берем цвета, нормализуем их (эмбиент и лайт)
// две тмп переменных, одна содержит цает объекта, вторая пустая
// для каждого (цвета лайта)
//		умножаем на оригинальный цвет объекта
// 		прибавляем к пустому цвету
